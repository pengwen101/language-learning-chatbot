...
if "messages_riasec" not in st.session_state:
    st.session_state.messages_riasec = [
        {"role": "assistant",
         "content": "Halo, apakah anda siap untuk mendapatkan rekomendasi karir?"}
    ]

if "chat_engine_riasec" not in st.session_state:

    memory = ChatMemoryBuffer.from_defaults(token_limit=50384)
    st.session_state.chat_engine_riasec = index.as_chat_engine(
    chat_mode="context",
    memory=memory,
...
for message in st.session_state.messages_riasec:
...
if prompt:= st.chat_input("What is up?"):
    ...
    st.session_state.messages_riasec.append({"role": "user", "content": prompt})
    ...
    with st.chat_message("assistant"):
        with st.spinner("Berpikir..."):
            response_stream = st.session_state.chat_engine_riasec.chat(prompt)
